<section>
  <div class="container">
    <div class="title-wrapper has-text-centered">
      <h2 class="title is-2">How to use it</h2>
      <div class="divider is-centered" />
    </div>
  </div>
  <div class="container">
    <div class="columns is-centered">
      <div class="column is-four-fifths">
        <div>
          <p class="help-text">Grab a copy of the template and install the dependencies</p>
          <code class="help-text">
            git clone https://github.com/jorgegorka/svelte-firebase my-app-name
            <br />
            cd my-app-name && yarn install
          </code>
        </div>
        <div>
          <p class="help-text">
            Add your Firebase configuration info to
            <em>src/config/firebase.js</em>
            (If you don't have a Firebase project you can create one in the
            <a href="https://firebase.google.com/" title="Firebase">Firebase website</a>
            )
          </p>
          <code class="help-text">
             {`const config = {`}
            <br />
             {`apiKey: '',`}
            <br />
             {`authDomain: '',`}
            <br />
             {`databaseURL: '',`}
            <br />
             {`projectId: '',`}
            <br />
             {`storageBucket: '',`}
            <br />
             {`messagingSenderId: ''`}
            <br />
             {`}`}
          </code>
          <p />
        </div>
        <div>
          <p class="help-text">Launch the development server</p>
          <code class="help-text">yarn dev</code>
        </div>
        <div>
          <p class="help-text">Visit http://localhost:5000</p>
          <p class="help-text">Enjoy!</p>
        </div>
        <div>
          <p class="help-text">For more detailed info read the documentation.</p>
        </div>
      </div>
    </div>
  </div>
</section>
